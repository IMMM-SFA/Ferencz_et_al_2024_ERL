...
historical_data = 'max' # choose min or max

monthly_use_data = pd.read_csv('Provider_historical_demands_' + historical_data + '.csv', index_col = 0) # modifies file name to load the min and max annual scanarios m
...
indoor_baseline = pd.read_csv("indoor_baseline.csv") 
indoor_baseline.columns = ['ID', 'indoor_acre_feet_per_month']
...
for n in range(len(providers)):
    ...
    if (provider in indoor_baseline.ID.values) != True or provider == 'IRR_SMT':
        continue 
    ...
    indoor_use = indoor_baseline.where(indoor_baseline.ID == provider).dropna()
    
    
    monthly_use = np.sum(hist_provider_monthly_use.iloc[:,2:14], axis = 0) # sum indoor and outdoor rows
    indoor_use = indoor_use.iloc[0,1] # indoor use acft/month factor 
    ...
    if provider in LU_SSP_ids and landclass_fractions.size > 0: # if provider region boundary is known and LC data is available 
        ...
        # to ensure that the SAME outdoor demand per unit green area factor is 
        # used for all scenarios for a given provider region 
        ...
        # outdoor_min_month = (1/12) * outdoor_area * 247.1 # convert area units to acres and multiply by 1/12 of a foot (conversion is 247.1 acres = 1 km^2)
        ...
        indoor_hist = indoor_use # acft/month for indoor use 
        indoor_hist_per_capita = indoor_hist/pop_hist # acft per person per month
        indoor_hist_per_capita_gpd = 325851 * 1/30 * indoor_hist_per_capita # gallons/day, factor of 325,851 converts acre-ft to gallons, and 1/30 converts gal/monthly to gal/day 
...
master_indoor_future_annual.to_csv('Provider_projection_indoor_' + historical_data + '_' + SSP + '_' + scenario + '.csv')
master_outdoor_future_annual.to_csv('Provider_projection_outdoor_' + historical_data + '_' + SSP + '_' + scenario + '.csv')
master_indoor_future_monthly.to_csv('Provider_projection_indoor_monthly_' + historical_data + '_' + SSP + '_' + scenario + '.csv')
master_outdoor_future_monthly.to_csv('Provider_projection_outdoor_monthly_' + historical_data + '_' + SSP + '_' + scenario + '.csv')
...
indoor_monthly.to_csv("Indoor_" + year + "_" + SSP + "_" + historical_data + "_" + scenario + "_with_pop.csv")
outdoor_monthly.to_csv("Outdoor_" + year + "_" + SSP + "_" + historical_data + "_" + scenario + "_with_pop.csv")
