...
outdoor_demand_proj_low = pd.read_csv("Provider_projection_outdoor_min_" + SSP + "_" + scenario + ".csv", index_col = 0)
outdoor_demand_proj_hi = pd.read_csv("Provider_projection_outdoor_max_" + SSP + "_" + scenario + ".csv", index_col = 0)
...
columns = ['ID', 'Pop_2010', 'Pop_2100', 'Outdoor_2010', 'Outdoor_2100', 'Outdoor_2100_low', 'Outdoor_2100_hi',
           'Indoor_2010', 'Indoor_2100', 'Initial_Weighted_LC','Outdoor_frac','grass_frac', 'tree_frac',
...
for n in range(len(outdoor_demand_proj.iloc[:,0])):
    ...
    if provider == 'IRR_SMT':
        ...
        if climate == True:
           ...
           provider_projection_outdoor = outdoor_demand_proj.where(outdoor_demand_proj.ID == provider).dropna()
           provider_projection_outdoor_low = outdoor_demand_proj_low.where(outdoor_demand_proj_low.ID == provider).dropna()
           provider_projection_outdoor_hi = outdoor_demand_proj_hi.where(outdoor_demand_proj_hi.ID == provider).dropna()
           
        provider_projection_indoor = indoor_demand_proj.where(indoor_demand_proj.ID == provider).dropna()
        columns = ['Pop_2010', 'Pop_2100', 'Outdoor_2010', 'Outdoor_2100', 'Outdoor_2100_low', 'Outdoor_2100_hi',
                   'Indoor_2010', 'Indoor_2100', 'Initial_Weighted_LC',
        ...
        demand_sensitivity.iloc[0,:] = np.array(([pop.iloc[0,1], pop.iloc[0,10], provider_projection_outdoor.iloc[0,1], 
                                                  provider_projection_outdoor.iloc[0,10], provider_projection_outdoor_low.iloc[0,10], 
                                                  provider_projection_outdoor_hi.iloc[0,10], provider_projection_indoor.iloc[0,1],
                                                  provider_projection_indoor.iloc[0,10], provider_projection_indoor.iloc[0,10], outdoor_frac.iloc[0,1],
...
outdoor_demand_change_low = np.divide((master_demand_sensitivity.Outdoor_2100_low[:] - master_demand_sensitivity.Outdoor_2010[:]), master_demand_sensitivity.Outdoor_2010[:])
outdoor_demand_change_hi = np.divide((master_demand_sensitivity.Outdoor_2100_hi[:] - master_demand_sensitivity.Outdoor_2010[:]), master_demand_sensitivity.Outdoor_2010[:])
...
total_demand_change_low = np.divide(((master_demand_sensitivity.Outdoor_2100_low[:] + master_demand_sensitivity.Indoor_2100[:]) - (master_demand_sensitivity.Outdoor_2010[:] + master_demand_sensitivity.Indoor_2010[:])), (master_demand_sensitivity.Outdoor_2010[:] + master_demand_sensitivity.Indoor_2010[:]))
total_demand_change_hi = np.divide(((master_demand_sensitivity.Outdoor_2100_hi[:] + master_demand_sensitivity.Indoor_2100[:]) - (master_demand_sensitivity.Outdoor_2010[:] + master_demand_sensitivity.Indoor_2010[:])), (master_demand_sensitivity.Outdoor_2010[:] + master_demand_sensitivity.Indoor_2010[:]))
...
for i in range(len(master_demand_sensitivity.iloc[:,0])):
    plt.vlines(100*pop_change.iloc[i], 100*total_demand_change_low.iloc[i], 100*total_demand_change_hi.iloc[i], color = 'k', alpha = .1)

plt.scatter(100*pop_change.values , 100*total_demand_change.values, c = np.divide(master_demand_sensitivity.Outdoor_2010[:],(master_demand_sensitivity.Outdoor_2010[:] + master_demand_sensitivity.Indoor_2010[:])), cmap = cm)
